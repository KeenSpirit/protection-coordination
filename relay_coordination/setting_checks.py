...
def check_settings(relays: list, triggers: list, percentage: float, f_type: str):
    ...
    if percentage < 1:
        if ((a == grading_check_iter and b < grading_check_iter)
            ...
            e = generate_settings(new_relays, percentage, f_type, grading_exact=True)
        else:
            e = generate_settings(new_relays, percentage, f_type, grading_exact=False)
        triggers = [a, b, c, d, e]
        return triggers

    a = generate_settings(new_relays, percentage, f_type, grading_exact=False)

    # Relax grading from 0.3s to the most exact grading margins
    if a == grading_check_iter:
        b = generate_settings(new_relays, percentage, f_type, grading_exact=True)
    ...
    if existing_relays and a == b == grading_check_iter:
        ...
        while (existing_relays and not all(grading_check)) and c_1 == grading_check_iter and c < grading_check_iter:
            ...
            new_relays = sorted(new_relays, key=lambda x: x.netdat.max_pg_fl)
            c_1 = generate_settings(new_relays, percentage, f_type, grading_exact=False)
            c += 1
    ...
    if a == b == c == grading_check_iter:
        d = generate_settings(new_relays, percentage, f_type, grading_exact=True)
...
def generate_settings(relays: list, percentage: float, f_type: str, grading_exact: bool):
    """
    ...
    :param f_type: 'EF', 'OC'
    :param grading_exact: True, False
    :param n: iterable
    :return:
    """
    n = 0
    ...
    while not all(grading_check) and n < grading_check_iter:
        if f_type == 'EF' and grading_exact is False:
            grading_check = [gs.generate_ef_settings(relays, percentage)]
            grading_check.extend([gm.ef_grade_time(relay) for relay in relays])
        elif f_type == 'EF' and grading_exact is True:
            grading_check = [gs.generate_ef_settings(relays, percentage)]
            grading_check.extend([gm.ef_grading_exact(relay) for relay in relays])
        elif f_type == 'OC' and grading_exact is False:
            grading_check = [gs.generate_oc_settings(relays, percentage)]
            grading_check.extend([gm.oc_grade_time(relay) for relay in relays])
        elif f_type == 'OC' and grading_exact is True:
            grading_check = [gs.generate_oc_settings(relays, percentage)]
            grading_check.extend([gm.oc_grading_exact(relay) for relay in relays])
        n += 1
    return n
