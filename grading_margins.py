from trip_time import oc_trip_time
from trip_time import ef_trip_time
...
def ef_grade_time(relay):
    ...
    if not relay.netdat.downstream_devices:
        ...
        for device in relay.netdat.downstream_devices:
            ...
            for x in b:
                trip_relay_1 = ef_trip_time(device, x)
                trip_relay_2 = ef_trip_time(relay, x)
    ...
    if not existing_upstream_ef:
        ...
        for device in existing_upstream_ef:
            ...
            for x in b:
                trip_relay_1 = ef_trip_time(relay, x)
                trip_relay_2 = ef_trip_time(device, x)
...
def oc_grade_time(relay):
    ...
    if not relay.netdat.downstream_devices:
        ...
        for device in relay.netdat.downstream_devices:
            ...
            for x in b:
                trip_relay_1 = oc_trip_time(device, x)
                trip_relay_2 = oc_trip_time(relay, x)
                grading_time_d = trip_relay_2 - trip_relay_1
                # Evaluate downstream grading against relay technology
                if device.manufacturer.technology in electro_m:
                    if grading_time_d >= electro_m[-1]:
    ...
    if existing_upstream_oc:
        for device in existing_upstream_oc:
            ...
            # For each fault level, calculate the grading time
            for x in b:
                trip_relay_1 = oc_trip_time(relay, x)
                trip_relay_2 = oc_trip_time(device, x)
                grading_time_u = trip_relay_2 - trip_relay_1
                if relay.manufacturer.technology in electro_m:
                    if grading_time_u >= electro_m[-1]:
                        grading_eval.append(True)
...
def ef_grading_exact(relay):
    ...
    if not relay.netdat.downstream_devices:
        ...
        for device in relay.netdat.downstream_devices:
            ...
            b = [a for a in range(device.netdat.min_pg_fl, device.netdat.max_pg_fl, 1)]
            for x in b:
                t_device = ef_trip_time(device, x)
                t_relay = ef_trip_time(relay, x)
                grading_actual = t_relay - t_device
                grading_required = (((2 * device.manufacturer.timing_error + device.ct.ect) / 100)
                                    * t_device + device.cb_interrupt + device.manufacturer.overshoot
    ...
    if not existing_upstream_ef:
        ...
        for device in existing_upstream_ef:
            ...
            for x in b:
                ...
                trip_relay_2 = ef_trip_time(device, x)
                grading_time_u = trip_relay_2 - trip_relay_1
                grading_required = ((2 * relay.manufacturer.timing_error + relay.ct.ect) / 100) * trip_relay_1 + relay.cb_interrupt \
                                   + relay.manufacturer.overshoot + relay.manufacturer.safety_margin
...
def oc_grading_exact(relay):
    ...
    if not relay.netdat.downstream_devices:
        ...
        for device in relay.netdat.downstream_devices:
            ...
            b = [a for a in range(device.netdat.min_2p_fl, device.netdat.max_3p_fl, 1)]
            for x in b:
                t_device = oc_trip_time(device, x)
                t_relay = oc_trip_time(relay, x)
                grading_actual = t_relay - t_device
                grading_required = (((2 * device.manufacturer.timing_error + device.ct.ect) / 100)
                                    * t_device + device.cb_interrupt + device.manufacturer.overshoot
                                    + device.manufacturer.safety_margin)
    ...
    if not existing_upstream_oc:
        ...
        for device in existing_upstream_oc:
            ...
            for x in b:
                ...
                grading_time_u = trip_relay_2 - trip_relay_1
                grading_required = (
                                           (2 * relay.manufacturer.timing_error + relay.ct.ect) / 100
                                   ) * trip_relay_1 + relay.cb_interrupt \
                                   + relay.manufacturer.overshoot + relay.manufacturer.safety_margin
                if grading_time_u >= grading_required:
                    grading_eval.append(True)
                else:
