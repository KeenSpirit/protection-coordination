...
def ef_report(best_relays):
    ...
    for relay in best_relays:
        ...
        trip_relay = ef_trip_time(relay, tr_max_pg)
        ...
        slowest_trip = 0
        for x in b:
            trip_relay = ef_trip_time(relay, x)
...
def oc_report(best_relays):
    ...
    for relay in best_relays:
        ...
        trip_relay = oc_trip_time(relay, tr_max_3p)
        fuse_grading_time = round(trip_relay - ds_melting_time, 3)
        max_fuse_grading = fuse_grading_time
        ...
        if inrush > relay.netdat.max_3p_fl:
            trip_inrush = round(oc_trip_time(relay, relay.netdat.max_3p_fl), 3)
        else:
            trip_inrush = round(oc_trip_time(relay, inrush), 3)
        ...
        slowest_trip = 0
        for x in b:
            trip_relay = oc_trip_time(relay, x)
            if trip_relay > slowest_trip:
                slowest_trip = round(trip_relay, 3)
        slowest_operate = slowest_trip
